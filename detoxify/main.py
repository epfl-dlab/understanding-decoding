...
def get_model_and_tokenizer(
    ...
    tokenizer = getattr(transformers, "AlbertTokenizerFast").from_pretrained(
...
def load_checkpoint(model_type="original", checkpoint=None, device="cpu", huggingface_config_path=None):
    if checkpoint is None:
        ...
        if 'hyper_parameters' in loaded:
            loaded['config'] = loaded['hyper_parameters']['config']
            # remove the prefix "model." from every key in the state_dict
            loaded['state_dict'] = {k[6:]: v for k, v in loaded['state_dict'].items()}
