...
def _get_information_extraction_prefix_allowed_tokens_fn_fairseq(
    ...
    def prefix_allowed_tokens_fn(batch_id, sent):
        ...
        # ---- If outside of a tag ----
        # Get allowed tokens
        allowed_tokens = get_allowed_tokens(sent, status_code)
        return allowed_tokens

    def get_allowed_tokens(sent, status_code):
        if status_code == "ob":
            allowed_tokens = [codes["start_of_tag"], codes["EOS"]]
        elif status_code == "s":
            allowed_tokens = _get_allowed_tokens(sent, entities_trie)
        elif status_code == "r":
            allowed_tokens = _get_allowed_tokens(sent, relations_trie)
        elif status_code == "o":
            allowed_tokens = _get_allowed_tokens(sent, entities_trie)
        else:
            raise RuntimeError
    ...
    def _get_allowed_tokens(sent, trie):
